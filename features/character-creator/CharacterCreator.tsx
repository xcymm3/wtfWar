...
export function CharacterCreator() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    if (!generatedSkills || attack === null || maxHealth === null) {
      ...
      <div className="creator-frame">
        <header className="creator-header">
          <div>
            ...
            <h1>给出设定，其余交给 AI。</h1>
            <p>选择职业与战力阶位，再写下名称和描述；AI 会生成符合战斗规则的属性与技能。</p>
          </div>
        ...
        <form className="creator-form" onSubmit={handleSubmit}>
          <section className="form-section">
            ...
            <div className="realm-heading">
              <span>职业</span>
              <small>职业会约束 AI 生成的基础属性范围。</small>
            </div>
          ...
          <section className="form-section generated-profile" aria-live="polite">
            <div className="section-heading">
              ...
              <div>
                <h2>自动生成的战斗配置</h2>
                <p>以下内容为只读结果；修改职业或角色描述后需要重新生成。</p>
              </div>
            </div>

            {generatedSkills && attack !== null && maxHealth !== null && effectiveStats ? (
              <>
                <div className="generated-attribute-grid">
                  <div><span>基础攻击</span><strong>{attack}</strong></div>
                  <div><span>基础生命</span><strong>{maxHealth}</strong></div>
                  <div><span>{REALM_LABELS[realm]}有效攻击</span><strong>{effectiveStats.attack}</strong></div>
                  <div><span>{REALM_LABELS[realm]}有效生命</span><strong>{effectiveStats.maxHealth}</strong></div>
                </div>
          ...
          <footer className="creator-actions">
            ...
            <button type="submit" disabled={!generatedSkills || isGenerating}>保存角色卡</button>
          </footer>
        </form>
      </div>
    </main>
